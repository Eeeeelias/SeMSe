    ...
    if type == 'conversation':
        for q in query_result:
            ...
            if part is None:
                ...
                combined_text = combine_parts(query_result, title, episode_id, timestamp)
                combined.append((title, episode_id, language, timestamp, combined_text, embedding, None))
        ...
        for q in query_result:
            ...
            if part is None:
                ...
                combined_text = combine_parts(query_result, title, episode_id)
                combined.append((title, episode_id, combined_text, embedding, None))
    return combined
...
def query_db(query: str, show: str = None, table: str = None,
             language: str = 'English', season=None, type='both', offset=0, conn=None):
    if not conn:
        conn = dbf.get_conn()
    else:
        conn = conn
...
def subtitle_query(conn, embed_query: np.ndarray, show: str = None,
    ...
    for idx, result in enumerate(results_sub):
        title, episode_id, _, timestamp, text, embedding, _ = result
